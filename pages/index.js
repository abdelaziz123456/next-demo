import Head from 'next/head'
import Image from 'next/image'
import styles from '../styles/Home.module.css';
import {serverSideTranslations} from 'next-i18next/serverSideTranslations';
import { withTranslation } from 'next-i18next';
import {  withRouter } from 'next/router';
import Link from 'next/link';
import { Component } from 'react';

import { i18n } from 'next-i18next';

export const getStaticProps = async ({ locale }) => ({
  props: {
    ...await serverSideTranslations(locale, ['common']),
  },
})

class Home extends Component{

  
  render(){


    

const {t}=this.props
  return (
    <div   >
      <Head>
 
        <title>Create Next App</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
     {
       console.log(this.props.i18n.changeLanguage())
     }

      <main className={`${styles.main}`}>
        <div className={styles.buttons}>
        <Link href={this.props.router.pathname} locale="ar" >
        <button>{t('Arabic')} </button>
        </Link>

    
           

        <Link href={this.props.router.pathname} locale="en" >
        <button >{t('English')}</button>
        </Link>





         
        </div>
      <button onClick={()=>{i18n.changeLanguage('ar')}}>ar</button>
       

        
          
       
        <h1 className={styles.title}>
         {t('Welcome to')}  <a href="https://nextjs.org">Next.js!</a>
        </h1>

        <p className={styles.description}>
        {('Get started by editing')}
          <code className={styles.code}>pages/index.js</code>
        </p>

        <div className={styles.grid} >
          <a href="https://nextjs.org/docs" className={styles.card}>
            <h2>{t('Documentation')}  </h2>
            <p>{t('Find in-depth information about Next.js features and API.')}  </p>
          </a>

          <a href="https://nextjs.org/learn" className={styles.card}>
            <h2>{t('Learn')}  </h2>
            <p>{t('Learn about Next.js in an interactive course with quizzes!')} </p>
          </a>

          <a
            href="https://github.com/vercel/next.js/tree/canary/examples"
            className={styles.card}
          >
            <h2>{t('Examples')}  </h2>
            <p>{t("Discover and deploy boilerplate example Next.js projects.")}</p>
          </a>

          <a
            href="https://vercel.com/new?utm_source=create-next-app&utm_medium=default-template&utm_campaign=create-next-app"
            className={styles.card}
          >
            <h2>{t("Deploy")}  </h2>
            <p>{t("Instantly deploy your Next.js site to a public URL with Vercel.")}
            
            </p>
          </a>
        </div>
      </main>

      <footer className={styles.footer}>
        <a
          href="https://vercel.com?utm_source=create-next-app&utm_medium=default-template&utm_campaign=create-next-app"
          target="_blank"
          rel="noopener noreferrer"
        >
          Powered by{' '}
          <span className={styles.logo}>
            <Image src="/vercel.svg" alt="Vercel Logo" width={72} height={16} />
          </span>
        </a>
      </footer>
    </div>
  )
}
}



export default withTranslation()(withRouter(Home));
